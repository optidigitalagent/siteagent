    ...
    def generate(self, query: str, project_name: str = None,
        ...
        return {
            ...
            "style": {
                ...
                "light_mode": best_style.get("Light Mode вњ“", ""),
                "dark_mode": best_style.get("Dark Mode вњ“", ""),
            },
            "colors": {
...
def hex_to_ansi(hex_color: str) -> str:
    """Convert hex color to ANSI True Color swatch (в–€в–€) with fallback."""
    ...
    return f"\033[38;2;{r};{g};{b}mв–€в–€\033[0m "
...
def section_header(name: str, width: int) -> str:
    """Create a Unicode section separator: в”њв”Ђв”Ђв”Ђ NAME в”Ђв”Ђв”Ђ...в”¤"""
    label = f"в”Ђв”Ђв”Ђ {name} "
    fill = "в”Ђ" * (width - len(label) - 1)
    return f"в”њ{label}{fill}в”¤"
...
def format_ascii_box(design_system: dict) -> str:
    ...
    lines.append("в•”" + "в•ђ" * w + "в•—")
    lines.append(ansi_ljust(f"в•‘  TARGET: {project} - RECOMMENDED DESIGN SYSTEM", BOX_WIDTH) + "в•‘")
    lines.append("в•љ" + "в•ђ" * w + "в•ќ")
    lines.append("в”Њ" + "в”Ђ" * w + "в”ђ")
    ...
    if any(dials.get(k) is not None for k in ("variance", "motion", "density")):
        ...
        if dials.get("variance") is not None:
            lines.append(f"в”‚  Variance: {dials['variance']}/10 вЂ” {dials['variance_label']}".ljust(BOX_WIDTH) + "в”‚")
        if dials.get("motion") is not None:
            lines.append(f"в”‚  Motion:   {dials['motion']}/10 вЂ” {dials['motion_label']}".ljust(BOX_WIDTH) + "в”‚")
        if dials.get("density") is not None:
            lines.append(f"в”‚  Density:  {dials['density']}/10 вЂ” {dials['density_label']}".ljust(BOX_WIDTH) + "в”‚")

    # Pattern section
    lines.append(section_header("PATTERN", BOX_WIDTH + 1))
    lines.append(f"в”‚  Name: {pattern.get('name', '')}".ljust(BOX_WIDTH) + "в”‚")
    if pattern.get('conversion'):
        lines.append(f"в”‚     Conversion: {pattern.get('conversion', '')}".ljust(BOX_WIDTH) + "в”‚")
    if pattern.get('cta_placement'):
        lines.append(f"в”‚     CTA: {pattern.get('cta_placement', '')}".ljust(BOX_WIDTH) + "в”‚")
    lines.append("в”‚     Sections:".ljust(BOX_WIDTH) + "в”‚")
    for i, section in enumerate(sections, 1):
        lines.append(f"в”‚       {i}. {section}".ljust(BOX_WIDTH) + "в”‚")

    # Style section
    lines.append(section_header("STYLE", BOX_WIDTH + 1))
    lines.append(f"в”‚  Name: {style.get('name', '')}".ljust(BOX_WIDTH) + "в”‚")
    light = style.get("light_mode", "")
    dark = style.get("dark_mode", "")
    if light or dark:
        lines.append(f"в”‚     Mode Support: Light {light}  Dark {dark}".ljust(BOX_WIDTH) + "в”‚")
    if style.get("keywords"):
        for line in wrap_text(f"Keywords: {style.get('keywords', '')}", "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")
    if style.get("best_for"):
        for line in wrap_text(f"Best For: {style.get('best_for', '')}", "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")
    if style.get("performance") or style.get("accessibility"):
        perf_a11y = f"Performance: {style.get('performance', '')} | Accessibility: {style.get('accessibility', '')}"
        lines.append(f"в”‚     {perf_a11y}".ljust(BOX_WIDTH) + "в”‚")
    ...
    for label, key, css_var in color_entries:
        ...
        content = f"в”‚     {swatch}{label + ':':14s} {hex_val:10s} ({css_var})"
        lines.append(ansi_ljust(content, BOX_WIDTH) + "в”‚")
    if colors.get("notes"):
        for line in wrap_text(f"Notes: {colors.get('notes', '')}", "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")

    # Typography section
    lines.append(section_header("TYPOGRAPHY", BOX_WIDTH + 1))
    lines.append(f"в”‚  {typography.get('heading', '')} / {typography.get('body', '')}".ljust(BOX_WIDTH) + "в”‚")
    if typography.get("mood"):
        for line in wrap_text(f"Mood: {typography.get('mood', '')}", "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")
    if typography.get("best_for"):
        for line in wrap_text(f"Best For: {typography.get('best_for', '')}", "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")
    if typography.get("google_fonts_url"):
        lines.append(f"в”‚     Google Fonts: {typography.get('google_fonts_url', '')}".ljust(BOX_WIDTH) + "в”‚")
    if typography.get("css_import"):
        lines.append(f"в”‚     CSS Import: {typography.get('css_import', '')[:70]}...".ljust(BOX_WIDTH) + "в”‚")

    # Key Effects section
    if effects:
        lines.append(section_header("KEY EFFECTS", BOX_WIDTH + 1))
        for line in wrap_text(effects, "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")

    # Motion section (GSAP skeleton, only if --motion dial was set)
    if motion_snippet:
        lines.append(section_header("MOTION", BOX_WIDTH + 1))
        lines.append(f"в”‚  {motion_snippet.get('Category', '')} ({motion_snippet.get('Intensity Tier', '')})".ljust(BOX_WIDTH) + "в”‚")
        lines.append(f"в”‚     Trigger: {motion_snippet.get('Trigger', '')} | Duration: {motion_snippet.get('Duration', '')} | Easing: {motion_snippet.get('Easing', '')}".ljust(BOX_WIDTH) + "в”‚")
        for line in wrap_text(f"GSAP: {motion_snippet.get('GSAP Snippet', '')}", "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")
        if motion_snippet.get("Framework Notes"):
            for line in wrap_text(f"Framework: {motion_snippet.get('Framework Notes', '')}", "в”‚     ", BOX_WIDTH):
                lines.append(line.ljust(BOX_WIDTH) + "в”‚")

    # Anti-patterns section
    if anti_patterns:
        lines.append(section_header("AVOID", BOX_WIDTH + 1))
        for line in wrap_text(anti_patterns, "в”‚     ", BOX_WIDTH):
            lines.append(line.ljust(BOX_WIDTH) + "в”‚")
    ...
    for item in checklist_items:
        lines.append(f"в”‚     {item}".ljust(BOX_WIDTH) + "в”‚")
...
def format_markdown(design_system: dict) -> str:
    ...
    if any(dials.get(k) is not None for k in ("variance", "motion", "density")):
        ...
        if dials.get("variance") is not None:
            lines.append(f"- **Variance:** {dials['variance']}/10 вЂ” {dials['variance_label']}")
        if dials.get("motion") is not None:
            lines.append(f"- **Motion:** {dials['motion']}/10 вЂ” {dials['motion_label']}")
        if dials.get("density") is not None:
            lines.append(f"- **Density:** {dials['density']}/10 вЂ” {dials['density_label']}")
        lines.append("")
    ...
    if motion_snippet:
        lines.append("### Motion")
        lines.append(f"**{motion_snippet.get('Category', '')}** ({motion_snippet.get('Intensity Tier', '')}) вЂ” Trigger: {motion_snippet.get('Trigger', '')} | Duration: {motion_snippet.get('Duration', '')} | Easing: `{motion_snippet.get('Easing', '')}`")
        lines.append("```js")
        ...
        if motion_do:
            lines.append(f"- вњ… {motion_do}")
        if motion_dont:
            lines.append(f"- вќЊ {motion_dont}")
        lines.append("")
...
def safe_slug(name, fallback: str = "default") -> str:
    ...
    (e.g. "../../etc") impossible вЂ” the slug can never leave its parent dir.
    """
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("")
    if spacing_scale:
        lines.append(f"*Density: {dials.get('density')}/10 вЂ” {dials.get('density_label')}*")
        lines.append("")
    ...
    if motion_snippet:
        ...
        if motion_snippet.get("Performance Notes"):
            lines.append(f"- вљЎ {motion_snippet.get('Performance Notes', '')}")
        lines.append("")
    ...
    if anti_patterns:
        ...
        for anti in anti_list:
            if anti:
                lines.append(f"- вќЊ {anti}")
    lines.append("")
    lines.append("### Additional Forbidden Patterns")
    lines.append("")
    lines.append("- вќЊ **Emojis as icons** вЂ” Use SVG icons (Heroicons, Lucide, Simple Icons)")
    lines.append("- вќЊ **Missing cursor:pointer** вЂ” All clickable elements must have cursor:pointer")
    lines.append("- вќЊ **Layout-shifting hovers** вЂ” Avoid scale transforms that shift layout")
    lines.append("- вќЊ **Low contrast text** вЂ” Maintain 4.5:1 minimum contrast ratio")
    lines.append("- вќЊ **Instant state changes** вЂ” Always use transitions (150-300ms)")
    lines.append("- вќЊ **Invisible focus states** вЂ” Focus states must be visible for a11y")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append("")
    lines.append("> вљ пёЏ **IMPORTANT:** Rules in this file **override** the Master file (`design-system/MASTER.md`).")
    ...
    if layout:
        ...
        lines.append("- No overrides вЂ” use Master layout")
    lines.append("")
    ...
    if spacing:
        ...
        lines.append("- No overrides вЂ” use Master spacing")
    lines.append("")
    ...
    if typography:
        ...
        lines.append("- No overrides вЂ” use Master typography")
    lines.append("")
    ...
    if colors:
        ...
        lines.append("- No overrides вЂ” use Master colors")
    lines.append("")
    ...
    if components:
        ...
        lines.append("- No overrides вЂ” use Master component specs")
    lines.append("")
...
def _generate_intelligent_overrides(page_name: str, page_query: str, design_system: dict) -> dict:
    ...
    if style_results:
        ...
        if any(kw in keywords.lower() for kw in ["data", "dense", "dashboard", "grid"]):
            ...
            spacing["Content Density"] = "High вЂ” optimize for information display"
            ...
            spacing["Content Density"] = "Low вЂ” focus on clarity"
        else:
